...
Each request executes one case phase using the existing evaluator, scoring, comparison and redaction code. D1 atomically claims the step and reserves every HTTP attempt before vendor dispatch, including retries. A unique account lock prevents competing starts or tabs from spending concurrently. Closing the screen pauses after the current step; reload and click Continue to resume. The overall deadline includes pauses; each phase has a 120-second bound. Cancel prevents additional calls; already-dispatched calls may finish and still be billed. A dead Worker leaves an uncertain phase: after its 150-second lease expires, the run becomes interrupted and that phase is never replayed. Attempts may be conservatively overcounted if execution died between reservation and dispatch. Missing usage remains unknown; provider invoices are authoritative.
