...
Server-side provider calls use `redirect: 'manual'` and reject every non-2xx response. The pinned Worker runtime rejects `redirect: 'error'` before making any request, despite Node fetch accepting it; that failure previously appeared as an unreachable provider. `pnpm test:worker` bundles the real adapters into workerd and checks all three vendors plus image/audio calls against a closed outbound fixture service. Its 56 requests cover successful responses, rejected credentials, and five redirect statuses without network access or real keys. CI runs this alongside the Node tests so request-option compatibility is exercised in the deployment runtime.
...
**New session** and choosing a different saved session ask before discarding an unsent question, current attachments (including pending reads), or instructions for a new, unsaved conversation. **Keep editing** preserves the draft; explicit discard clears it and invalidates late attachment reads. Selecting the already-open session is harmless. Drafts and file bytes are not automatically persisted. A browser navigation warning is registered while a draft or run is active, subject to browser support and prior user interaction; completing/discarding the draft or stopping an otherwise empty run removes it. Delete, clear-history, and account-reload confirmations also disclose when drafts will be cleared. `node tests/browser-question-drafts.mjs` checks guest/account flows, a real cancelled refresh, mobile, and file-read races locally without provider calls or account writes.
...
Personal memory has its own portable backup, separate from conversation backups and their historical memory snapshots. In **Personal memory → Back up personal memory**, **Download memory** exports the current notes and enable setting; if they differ from saved values, **Download memory draft** explicitly exports those unsaved edits without saving them to the account. The JSON file is unencrypted. Notes may contain private text: keep the file private and avoid recording credentials in memory.

**Choose memory backup** validates a versioned `trio-personal-memory` file under 32 KB and previews its date, notes, and former enable setting. No account or provider request occurs while choosing or previewing. **Replace draft with imported notes** replaces only the editor draft and sets memory to off regardless of the file's old setting. Review the notes, deliberately enable memory if desired, and choose **Save memory** to use the normal account-pinned, revision-checked save. Restoring cannot copy a foreign account identity or revision, overwrite concurrent changes, or automatically share notes with models. Selecting, cancelling, or rejecting a backup preserves existing notes. Closing with a pending preview or edited draft asks before discarding; stale file reads are ignored.

The file includes only `format`, `version`, `exportedAt`, and `memory` (`notes`, `enabled`). Unknown versions/fields, malformed JSON, excessive text, or conversation backups are rejected in full. Empty notes and disabled memory can be backed up. Account IDs, server revisions, connection settings, and original attachments are not exported. All 4,000 supported characters, including JSON escapes and Unicode, fit the file limit. Run `node tests/browser-memory-backups.mjs` locally to check export, preview, safe restore, cancellation, conflict handling, and mobile layout without provider calls.
...
**Create image** opens a separate description editor. In Live mode with OpenAI enabled and an API key, choose square/landscape/portrait and low/medium/high quality, then explicitly **Generate with OpenAI**. One image is generated with `gpt-image-2.5-flare` as an opaque JPEG at compression 90. Each attempt is separately billed by OpenAI and excluded from answer usage totals; higher quality and larger images can cost more. The fixed Images API receives only the description and image settings, never conversation history, memory, attachments, or another vendor's key. Access depends on the user's OpenAI permissions and account verification.

Preview and **Download JPEG**, or **Attach to question** to use the existing image-understanding workflow. Attaching preserves question text; if another image is attached the action explicitly says **Replace attached image**. The image plus any existing PDF must fit 4 MB. The generated image is not sent to other models until Ask Trio. Generated descriptions/media are not saved to account history or browser storage; only the attachment's generated filename enters a completed answer. Download files to keep them. Generation can invent details and misspell text, and peer review does not guarantee accuracy.

`POST /api/images/generate` requires authenticated identity, a matching account header and same origin. Its strict 32 KB JSON input accepts a 4,000-character description and allowlisted settings. The adapter posts once to `https://api.openai.com/v1/images/generations`, forbids redirects/caches, uses default moderation, discards vendor diagnostics, and never downloads returned URLs. It reads at most 6 MB of JSON, validates canonical JPEG bytes under 4 MB, and the browser decodes the raster before display. Other provider JSON readers retain the 2 MB default. Server and browser deadlines are 180/210 seconds. No automatic retries occur; Stop/dismissal/unmount invalidate late responses. Failure or cancellation retains an earlier successful preview. Unsaved descriptions/results have discard and navigation warnings. Opening Connections preserves an image description for reopening the tool.

The request contract follows [OpenAI's image-generation guide](https://developers.openai.com/api/docs/guides/image-generation) and [Images API reference](https://developers.openai.com/api/reference/cli/resources/images/methods/generate). Unit, local browser, and local Worker checks use synthetic images and simulated responses; paid generation and output quality remain unverified without a real API key.
...
## Private in-app quality checks

Open **Quality check** in the signed-in workspace sidebar. Save and enable at least two provider keys, switch to Live mode, and review the suite, team mode, fixed comparison baseline, nominal calls, hard HTTP-attempt cap (1–500), and wall-clock deadline (1–3600 seconds; default 900). Defaults match the CLI: core suite, Council, Claude baseline, 60 attempts. An illustrative token-based dollar estimate is shown only for known, unexpired configured rates; it is not a quote or dollar spending limit. Providers bill the account owner directly. Click **Start paid quality check** to authorize the run; opening the screen, reading results, or downloading files does not call providers.

The account vault resolves saved keys on the server using the authenticated identity, account pin, same-origin checks, and frozen credential revisions. No key is supplied by the quality-check browser payload or returned to it. A run freezes its cases and model IDs. Changing/deleting a participating saved connection stops the run rather than silently changing its comparison. Demo mode cannot start/continue a check.

Each request executes one case phase using the existing evaluator, scoring, comparison and redaction code. D1 atomically claims the step and reserves every HTTP attempt before vendor dispatch, including retries. A unique account lock prevents competing starts or tabs from spending concurrently. Closing the screen pauses after the current step; reopen and click Continue to resume. A claimed phase uses a server-owned signal rather than the browser request signal. The route awaits it and also registers it with `waitUntil`, which provides at most 30 seconds of post-disconnect execution. Refreshing or closing the browser tab can therefore interrupt a longer phase if the platform kills it; saved steps remain available as a partial report and are never silently replayed. See [Cloudflare execution-context limits](https://developers.cloudflare.com/workers/runtime-apis/context/). The overall deadline includes pauses; each phase has a 120-second bound. Cancel prevents additional calls; already-dispatched calls may finish and still be billed. A dead Worker leaves an uncertain phase: after its 150-second lease expires, the run becomes interrupted and that phase is never replayed. Attempts may be conservatively overcounted if execution died between reservation and dispatch. Missing usage remains unknown; provider invoices are authoritative.

Private quality runs and phase results live in separate D1 tables, outside conversation history, backups and memory. The screen lists the latest 30 checks, displays per-case outcomes, and exports the full report. When a run has stopped, download the **blinded sheet** and **identity key** separately; a persisted random seed keeps their shuffled labels consistent across downloads. Grade before opening the key or the unblinded results. These synthetic exact-answer tasks do not establish general accuracy, business usefulness, absence of bias, or that collaboration caused an error/correction. No public product claims are computed from these reports.

`node tests/worker-quality-check.mjs` exercises the real route, D1, WebCrypto, evaluator and provider transports against synthetic outbound responses; no real network or credentials are used. It covers identity/origin isolation, idempotency, competing steps, call budgets, cancellation, expiry, redaction and stable blind exports. `node tests/browser-quality-check.mjs` checks the interactive flow against fixture quality responses, including refresh/resume and mobile layout.
