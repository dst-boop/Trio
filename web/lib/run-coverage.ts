...
export function runCoverage(result: Result, mode: Mode): CoverageStep[] {
  ...
  const steps: CoverageStep[] = [
    ...
    { phase: 'synthesis', label: 'Write answer', state: mode === 'single' ? 'skipped' : result.fallback ? 'unavailable' : result.answer.trim() ? 'complete' : mode === 'compare' ? 'skipped' : 'unavailable', detail: mode === 'single' ? 'The selected model’s original answer is shown directly. No synthesis call was made.' : result.fallback ? 'Synthesis did not complete. A single model’s draft or revision is shown as the fallback.' : mode === 'compare' ? 'Compare keeps the perspectives separate; no combined answer was requested.' : result.answer.trim() ? `${drafts.length > 1 ? 'The model combined the available contributions' : 'One model wrote the answer'}. This is not a guarantee of correctness.` : 'No final answer was returned.' },
  ];
