...
export type Turn = { question: string; result: Result; mode: Mode };
...
export function conversationHistory(turns: Turn[]): { role: 'user' | 'assistant'; content: string }[] {
  return turns.filter(t => !t.result.demo).slice(-6).flatMap(t => {
    ...
    return answer ? [{ role: 'user' as const, content: t.question }, { role: 'assistant' as const, content: answer.slice(0, 30000) }] : [];
  });
}
