...
export async function suggestMemory(session: Session, existing: string, connection: z.infer<typeof memoryConnectionSchema>, signal: AbortSignal, fetcher: typeof fetch = fetch): Promise<string> {
  const conversation = session.turns.filter(t => !t.result.demo).slice(-6).map(t => ({ user: t.question.slice(0, 4000), assistant: memoryAnswer(t), ...(t.feedback ? { user_feedback: { rating: t.feedback.rating, note: t.feedback.note?.slice(0, 2000) } } : {}) })).filter(t => t.assistant);
