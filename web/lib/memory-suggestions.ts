...
export async function suggestMemory(session: Session, existing: string, connection: z.infer<typeof memoryConnectionSchema>, redactionKeys: string[], signal: AbortSignal, fetcher: typeof fetch = fetch): Promise<string> {
  ...
  const conversation = session.turns.filter(t => !t.result.demo).slice(-6).map(t => {
    // Mask complete source strings before excerpting can split a credential.
    const safeTurn = { ...t, result: { ...t.result, answer: redact(t.result.answer), drafts: Object.fromEntries(Object.entries(t.result.drafts).map(([provider, text]) => [provider, redact(text!)])) } };
    return { user: redact(t.question).slice(0, 4000), assistant: conversationAnswerExcerpt(safeTurn, 8000).text, ...(t.feedback ? { user_feedback: { rating: t.feedback.rating, note: t.feedback.note ? redact(t.feedback.note).slice(0, 2000) : undefined } } : {}) };
