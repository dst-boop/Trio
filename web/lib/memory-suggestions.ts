...
import { conversationAnswer, type Session } from './sessions.ts';
...
export async function suggestMemory(session: Session, existing: string, connection: z.infer<typeof memoryConnectionSchema>, signal: AbortSignal, fetcher: typeof fetch = fetch): Promise<string> {
  const conversation = session.turns.filter(t => !t.result.demo).slice(-6).map(t => ({ user: t.question.slice(0, 4000), assistant: conversationAnswer(t).slice(0, 8000) })).filter(t => t.assistant);
