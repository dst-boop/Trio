...
export function evaluationConfig(args: string[], env: Record<string, string | undefined>) {
  ...
  for (let i = 0; i < args.length; i++) {
    const flag = args[i];
    if (!['--run', '--help', '--include-answers', '--providers', '--mode', '--cases', '--max-calls', '--timeout-seconds', '--output'].includes(flag) || flags.has(flag)) throw new Error('Unknown or repeated evaluation option. Use --help.');
  ...
  const ids = flags.get('--cases')?.split(',') ?? qualityCases.map(c => c.id);
  if (!ids.length || new Set(ids).size !== ids.length || ids.some(id => !qualityCases.some(c => c.id === id))) throw new Error('Select distinct case IDs shown in the preview.');
  ...
  const options: EvaluationOptions = { mode: mode as EvaluationOptions['mode'], maxCalls: number('--max-calls', 60, 500), timeoutSeconds: number('--timeout-seconds', 900, 3600), includeAnswers: flags.has('--include-answers'), cases: ids.map(id => qualityCases.find(c => c.id === id)!) };
