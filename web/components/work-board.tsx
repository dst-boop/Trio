...
export function WorkBoard({ sessions, busy, hasDraft, onPrepareDay, onFocusComposer, open, onOpenChange, onSave, onOpenSession }: { sessions: Session[]; busy: boolean; hasDraft: boolean; onPrepareDay: (prompt: string) => boolean; onFocusComposer: () => void; open: boolean; onOpenChange: (open: boolean) => void; onSave: (id: string, index: number, plan: WorkPlan | null) => boolean; onOpenSession: (id: string) => void }) {
  ...
  const plansTab = useRef<HTMLButtonElement | null>(null);
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="work-dialog work-board" onCloseAutoFocus={event => { if (prepared.current) { event.preventDefault(); prepared.current = false; onFocusComposer(); } }}><DialogTitle>Your work</DialogTitle><DialogDescription>Choose your next action, update a plan, or record what happened. Generating an answer does not complete real-world work.</DialogDescription>
    ...
    <Tabs value={tab} onValueChange={setTab} className="work-board-tabs"><TabsList aria-label="Your work views"><TabsTrigger value="actions">Next actions</TabsTrigger><TabsTrigger value="plans" ref={plansTab}>Plans</TabsTrigger><TabsTrigger value="results">Results · 7 days</TabsTrigger></TabsList>
    <TabsContent value="actions"><NextActions sessions={sessions} busy={busy} hasDraft={hasDraft} onPrepare={prompt => { prepared.current = true; if (!onPrepareDay(prompt)) { prepared.current = false; return false; } onOpenChange(false); return true; }} onSave={onSave} filter={actionFilter} setFilter={setActionFilter} search={actionSearch} setSearch={setActionSearch} onOpenPlan={(id, index) => { setFilter('all'); setSelectedPlan(JSON.stringify([id, index])); plansTab.current?.focus(); setTab('plans'); }} /></TabsContent>
    ...
    <label>Show plans<select aria-label="Show plans" value={filter} onChange={e => setFilter(e.target.value)}><option value="open">In progress</option><option value="all">All plans and outcomes</option></select></label>
    ...
    <div className="work-board-plans">{visible.map(({ session, turn, index }) => { const key = JSON.stringify([session.id, index]); return <details data-plan-key={key} key={key} open={selectedPlan === key} onToggle={event => { if (event.currentTarget.open) setSelectedPlan(key); else setSelectedPlan(current => current === key ? null : current); }}><summary><span>{turn.work!.goal}</span><small>{actions.filter(a => a.sessionId === session.id && a.turnIndex === index).length} open actions{turn.work!.outcome ? ' · Outcome recorded' : ''}</small></summary><button className="subtle-button" disabled={busy} onClick={() => { onOpenChange(false); onOpenSession(session.id); }}>Open conversation: {session.title.slice(0, 80)}</button><WorkPlanPanel reviewed={!!turn.result.reviewedAnswer} value={turn.work} question={turn.question} busy={busy} onSave={plan => onSave(session.id, index, plan)} /></details>; })}</div>
    </TabsContent></Tabs>
