...
export function SessionActionDialogs({ action, sessions, busy, onClose, onRename, onDelete }: { action: SessionAction; sessions: Session[]; busy: boolean; onClose: () => void; onRename: (id: string, title: string) => void; onDelete: (id: string) => void }) {
  ...
  return <>
    ...
    <AlertDialog open={action?.type === 'delete' && !!session} onOpenChange={open => { if (!open) onClose(); }}><AlertDialogContent className="session-action-dialog"><AlertDialogTitle>Delete this session?</AlertDialogTitle><AlertDialogDescription>This removes “{titlePreview}” and its {session?.turns.length} completed questions from this workspace’s saved history. Export it first if needed. Downloaded backups remain unchanged.</AlertDialogDescription><AlertDialogFooter><AlertDialogCancel>Keep session</AlertDialogCancel><AlertDialogAction variant="destructive" disabled={busy} onClick={() => { if (session && !busy) onDelete(session.id); }}>Delete session</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </>;
}
