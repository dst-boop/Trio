...
export default function Home({ account }: { account?: { userId: string; displayName: string; email: string } }) {
  ...
  return <SidebarProvider style={{ '--sidebar-width': '248px' } as React.CSSProperties}>
    ...
    <Dialog open={toolsOpen} onOpenChange={setToolsOpen}><DialogContent className="tools-dialog" onCloseAutoFocus={event => { event.preventDefault(); promptRef.current?.focus(); }}><DialogTitle>Tools &amp; context</DialogTitle><DialogDescription>Add source material, instructions, research, or a guided work brief.</DialogDescription>        <div className="question-options">
