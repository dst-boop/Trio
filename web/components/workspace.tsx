  ...
  return <SidebarProvider style={{ '--sidebar-width': '248px' } as React.CSSProperties}>
    <Sidebar className="trio-sidebar">
      ...
      <SidebarContent className="side-content">
        <button className="new-session" onClick={newSession} disabled={busy}><Plus size={17} /> New session <span>↗</span></button>
    ...
    <main className="workspace">
      ...
      <div className="work-body">
        ...
        {displayed && <section className="results-section"><div className="question-title"><MessageSquare size={17} /><h2>{question}</h2></div>{displayed.demo && <div className="demo-notice">ILLUSTRATIVE DEMO · Prepared sample responses. No model APIs were called.</div>}<RunPipeline result={displayed} mode={displayMode} busy={busy} stage={stage} />{!busy && !working && <RunCoverage result={displayed} mode={displayMode} />}
          ...
          <Tabs value={tab} onValueChange={setTab}>{busy && !demo && <p className="revision-note">Responses are arriving live. Partial text may change; only completed runs are saved.</p>}<div className="result-toolbar"><TabsList className="result-tabs"><TabsTrigger value="answer">Synthesis</TabsTrigger><TabsTrigger value="drafts">Perspectives <span>{Object.keys(displayed.drafts).length}</span></TabsTrigger><TabsTrigger value="reviews">Peer reviews <span>{Object.keys(displayed.reviews).length}</span></TabsTrigger>{displayMode === 'deep' && <TabsTrigger value="revisions">Revisions <span>{Object.keys(displayed.revisions ?? {}).length}</span></TabsTrigger>}</TabsList><div className="result-actions">{!working && turns.length > 0 && <button className="branch-latest" disabled={busy} onClick={() => setBranchPoint(turns.length - 1)}>Continue from here ↗</button>}<button aria-label="Copy answer" disabled={!displayed.answer} onClick={() => void copy(displayed.answer)}><Copy size={16} /></button><button aria-label="Export session as Markdown" disabled={!turns.length || busy} onClick={exportSession}><Download size={16} /></button></div></div>
