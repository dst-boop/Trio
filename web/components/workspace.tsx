  ...
  return <SidebarProvider style={{ '--sidebar-width': '248px' } as React.CSSProperties}>
    ...
    <main className="workspace">
      ...
      <div className="work-body">
        ...
        <div className="connection-summary"><span>{demo ? 'Prepared example · no live answers' : connected + ' model' + (connected === 1 ? '' : 's') + ' connected'}</span><button disabled={busy} onClick={() => { if (demo && connected) setDemo(false); else setSettings(true); }}>{demo ? connected ? 'Use live models' : 'Set up live answers' : 'Manage connections'}</button></div>
        {displayed && <section className="results-section"><div className="question-title"><MessageSquare size={17} /><h2>{question}</h2></div>{displayed.demo && <div className="demo-notice">ILLUSTRATIVE DEMO · Prepared sample responses. No model APIs were called.</div>}<RunPipeline result={displayed} mode={displayMode} busy={busy} stage={stage} />{!busy && !working && <RunCoverage result={displayed} mode={displayMode} />}
          ...
          {!working && current && !turns.at(-1)?.result.demo && <WorkPlanPanel reviewed={!!turns.at(-1)?.result.reviewedAnswer} key={'work:' + current + ':' + (turns.length - 1)} value={turns.at(-1)?.work} question={turns.at(-1)?.question ?? ''} busy={busy} onSave={plan => saveWork(current, turns.length - 1, plan)} />}
        ...
        <section className="collaboration-settings answer-settings"><label>Answer mode<select aria-label="Answer mode" value={mode} disabled={busy} onChange={e => setMode(e.target.value as Mode)}>{Object.entries(modes).map(([key, m]) => <option key={key} value={key}>{m.title}</option>)}</select></label><label>{mode === 'single' ? 'Answer model' : 'Final writer'}<select aria-label="Answer model" value={lead} disabled={busy} onChange={e => setLead(e.target.value as ProviderId)}>{providers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><div className="flow-label"><small>{modes[mode].desc}</small><small>{demo ? 'Prepared example · no API usage' : modes[mode].calls + (webResearch ? ' · research adds calls and search fees' : '')}</small></div></section>
        ...
        <WorkflowBriefs busy={busy} prompt={prompt} onApply={text => { setPrompt(text); setDemo(false); promptRef.current?.focus(); toast('Brief prepared. Review your mode and connections, then choose Ask Trio.'); }} />
