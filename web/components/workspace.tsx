...
export default function Home({ account }: { account?: { userId: string; displayName: string; email: string } }) {
  ...
  return <SidebarProvider style={{ '--sidebar-width': '248px' } as React.CSSProperties}>
    ...
    <main className="workspace">
      <header className="topbar"><div className="breadcrumb"><SidebarTrigger className="mobile-menu" /><span>Workspace</span><ChevronRight size={14} /><strong>New possibilities</strong></div><div className="top-actions">{displayed && <button className="subtle-button question-jump" onClick={() => { document.getElementById("question-composer")?.scrollIntoView({ block: "start" }); if (!busy) promptRef.current?.focus({ preventScroll: true }); }}>Question &amp; modes ↑</button>}{busy && <button className="subtle-button stop-current" onClick={() => abortRef.current?.abort()}><Square size={14} />Stop current run</button>}<span className={`mode-badge ${demo ? '' : 'live'}`}>{demo ? 'Demo workspace' : 'Live workspace'}</span><button className="subtle-button" aria-label="Connections" onClick={() => setSettings(true)}><Settings2 size={15} /><span>Connections</span></button></div></header>
      <div className="work-body">
        ...
        <details className="question-options"><summary>Guided workflows</summary><WorkflowBriefs busy={busy || preferences.loading} prompt={prompt} onApply={text => { setPrompt(text); setDemo(false); promptRef.current?.focus(); toast('Brief prepared and Live mode selected. Review your model choices, then choose Ask Trio.'); }} /></details>
