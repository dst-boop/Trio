...
import { savedKeyReference, type SavedConnection } from '@/lib/saved-connections';
import { AlertDialog, AlertDialogContent, AlertDialogTitle, AlertDialogDescription, AlertDialogFooter, AlertDialogCancel, AlertDialogAction } from '@/components/ui/alert-dialog';

export function ProviderConnection({ id, connection, disabled, open, onChange, accountId, saved, saving, canSave, onSave, onRemove }: { id: ProviderId; connection: Connections[ProviderId]; disabled: boolean; open: boolean; onChange: (value: Connections[ProviderId]) => void; accountId?: string; saved?: SavedConnection; saving?: boolean; canSave?: boolean; onSave?: () => void; onRemove?: () => void }) {
  ...
  return <div className="connection-row">
    ...
    <label>API key<input type="password" autoComplete="off" spellCheck={false} disabled={disabled} maxLength={1024} placeholder={usingSaved ? 'Saved securely · paste to replace' : 'Paste your API key'} value={usingSaved ? '' : connection.key} onChange={e => change({ ...connection, key: e.target.value.trim() || (saved?.saved ? savedKeyReference : '') })} /></label>
