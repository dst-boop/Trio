...
try {
  ...
  const beforeHandoff = await read();
  await board.getByRole('button',{name:'Prepare a day plan',exact:true}).click();
  await page.getByRole('spinbutton',{name:'Available minutes (optional)',exact:true}).fill('60');
  await page.getByRole('spinbutton',{name:'Available minutes (optional)',exact:true}).press('Enter');
  assert.ok(await page.getByRole('dialog',{name:'Plan from your saved actions',exact:true}).isVisible());
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  await page.getByRole('button',{name:'Keep editing',exact:true}).click();
  await page.waitForFunction(()=>document.activeElement?.textContent==='Cancel');
  assert.equal(await page.getByRole('spinbutton',{name:'Available minutes (optional)',exact:true}).inputValue(),'60');
  await page.getByRole('button',{name:'Use in new Live conversation',exact:true}).click();
  await page.waitForFunction(()=>document.activeElement?.getAttribute('aria-label')==='Your question');
  const dayBrief=await page.getByLabel('Your question',{exact:true}).inputValue();
  assert.match(dayBrief,/"available_minutes": 60/);assert.match(dayBrief,/Review and send the follow-up/);
  assert.deepEqual((await read()).sessions,beforeHandoff.sessions);assert.equal(calls,0);
