...
try{
  for(const account of [false,true]){
    ...
    try{
      ...
      if(account){snapshot=await(await page.request.get(base+'/api/workspace')).json();headers={Origin:base,'X-Trio-Account':snapshot.accountId,'X-Trio-Workspace-Version':'3'};assert.equal((await page.request.put(base+'/api/workspace',{headers,data:{revision:snapshot.revision,sessions:fixtures}})).status(),200);await page.goto(base+'/workspace',{waitUntil:'networkidle'});}
