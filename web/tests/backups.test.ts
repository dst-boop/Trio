...
test('portable backups round-trip complete conversations and strip unknown fields at every level', () => {
  ...
  assert.equal(parsed.format, 'trio-workspace'); assert.equal(parsed.version, 6);
});

test('V5 exports are rejected by the legacy version gate, while all V1 data remains readable', () => {
  ...
  const upgraded = exportBackup(parsed); assert.equal(JSON.parse(upgraded).version, 6); assert.deepEqual(parseBackup(upgraded), parsed);
  ...
  assert.equal(z.object({ version: z.union([z.literal(1), z.literal(2)]) }).safeParse(JSON.parse(current)).success, false);
...
test('unsupported, partial, invalid, and duplicate-ID files are rejected as a whole', () => {
  ...
  for (const value of [null, [], { ...good, version: 7 }, { ...good, format: 'other' }, { ...good, exportedAt: 'bad-date' }, { ...good, sessions: [] }, { ...good, sessions: [...good.sessions, { ...session('bad'), turns: [null] }] }, { ...good, sessions: [session(), session()] }, { ...good, sessions: Array.from({ length: 31 }, (_, i) => session(String(i))) }]) assert.throws(() => parseBackup(JSON.stringify(value)), /supported Trio backup/);
